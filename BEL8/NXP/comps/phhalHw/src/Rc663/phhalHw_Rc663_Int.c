...
* $Revision: 3068 $ (NfcRdLib_A_Gate_Incremental_V3.092.04.001533 : 3005 )
                                     ...
                                     )
{
    ...
    default:
        ...
        if (bCmdNoWrite)
        {
            ...
            bIrq0WaitFor |= PHHAL_HW_RC663_BIT_IRQINV;
...
#ifdef NXPBUILD__PHHAL_HW_RC663_DEPRECATED
                                        ...
                                        )
{
    ...
    do{
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TESTMOD, 0x80));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_TESTMOD, &bReg));
    }while(bReg != 0x80);

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TESTMOD, 0xC0));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_SIGPROTEST, 0x40));

    return statusTmp;
}
#endif /* NXPBUILD__PHHAL_HW_RC663_DEPRECATED */
                                    ...
                                    )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, 0x66, 0x80));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, 0x66, &bReg));

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, 0x66, 0xC0));

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, 0x66, 0xE0));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, 0x66, &bReg));

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, 0x66, 0xD0));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, 0x66, &bReg));

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, 0x66, 0xC3));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, 0x66, &bReg));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, 0xB2, 0x40));
...
phStatus_t phhalHw_Rc663_CheckForEmdError(phhalHw_Rc663_DataParams_t * pDataParams, uint8_t bIrq0WaitFor, uint8_t bIrq1WaitFor, uint32_t * pdwMultiReg, uint32_t * pdwPrevReg)
{
    ...
    do {
            ...
            if( (PHHAL_HW_EMD_NOISE_CHECK(aTempRxBuf[3]) && (aTempRxBuf[1] & PHHAL_HW_RC663_BIT_RXIRQ))&&
                ( ((aTempRxBuf[4] & 0xFF) <  (pDataParams->bEmdNoiseMaxDataLen)) || (bRxBitCtrl != 0x00) ))
            {
                /* Restart reception */
                                                 ...
                                                 )
{
    ...
    uint16_t    PH_MEMLOC_REM wRxLength;
    uint8_t     PH_MEMLOC_REM bRxLastBits;
    uint8_t     PH_MEMLOC_REM bDr;
    uint8_t     PH_MEMLOC_REM bM;

    bDr = (pBeginRndCmd[0] & (1 << 3)) >> 3;
    bM = (pBeginRndCmd[0] & (3 << 1)) >> 1;
    pDataParams->bSession = pBeginRndCmd[1] & (3 << 3);

    /* Set Manchester-2 coding */
    if (bM == PHHAL_HW_RC663_I18000P3M3_M_MANCHESTER_2)
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
            pDataParams,
