...
* $Revision: 3068 $ (NfcRdLib_A_Gate_Incremental_V3.092.04.001533 : 3021 )
                                     ...
                                     )
{
    ...
    /* Build Command frame */
    bCmdLength = 0;
    bCmd[bCmdLength++] = (PHPAL_I18000P3M3_CMD_SELECT << 4) | (bTarget << 1) | (bAction >> 2);  /* 0 bits left */
    bCmd[bCmdLength]   = (bAction << 6) | (bMemBank <<  4) | (bPointerLength << 2);             /* 2 bits left */

    /* append pointer */
    for (bIndex = 0; bIndex < (bPointerLength + 1); ++bIndex)
    {
        bCmd[bCmdLength++] |= (pPointer[bIndex]) >> 6;
        bCmd[bCmdLength]    = (pPointer[bIndex]) << 2;
    }
    /* 2 bits left */

    bCmd[bCmdLength++] |= bMaskBitLength >> 6;
    bCmd[bCmdLength]    = bMaskBitLength << 2;  /* 2 bits left */
    ...
    for (bIndex = 0; bIndex < bMaskByteLength; ++bIndex)
    {
        ...
        /* Append mask value */
        bCmd[bCmdLength++] |= bTmp >> 6;
        bCmd[bCmdLength]    = bTmp << 2;
    }
    ...
    if (bTruncate != PH_OFF)
    {
        bCmd[bCmdLength] |= (uint8_t)(0x80 >> (wCmdBitLength & 0x07));
    }
    ++bCmdLength;
    ++wCmdBitLength;
    ...
    )
{
    ...
    /* Build Command frame */
    aCmd[0] = (PHPAL_I18000P3M3_CMD_BEGIN_ROUND << 4) | (bDr << 3) | (bM << 1) | (bTRext);  /* 0 bits left */
    aCmd[1] = (bSel << 6) | (bSession << 4) | (bRfu << 3) | (bQ >> 1);                      /* 0 bits left */
    aCmd[2] = (bQ << 7);                                                                    /* 7 bits left */
    ...
    /* Return StoredCRC */
    memcpy(pStoredCRC, pResponse, wRxLength);  /* PRQA S 3200 */

    /* Save StoredCRC */
    memcpy(pDataParams->abStoredCRC, pResponse, wRxLength);  /* PRQA S 3200 */
    ...
    )
{
    ...
    /* Build Command frame */
    aCmd[0] = (PHPAL_I18000P3M3_CMD_RESIZE_ROUND << 4) | (pDataParams->bSession << 2) | (bUpDn >> 1);   /* 0 bits left */
    aCmd[1] = (bUpDn << 7);                                                                             /* 7 bits left */
                                       ...
                                       )
{
    ...
    /* Build Command frame */
    aCmd[0] = (PHPAL_I18000P3M3_CMD_NEXT_SLOT << 4) | (pDataParams->bSession << 2); /* 2 bits left */
    ...
    )
{
    ...
    if (!pDataParams->bStoredCRCValid)
    {
        return PH_ADD_COMPCODE(PH_ERR_USE_CONDITION, PH_COMP_PAL_I18000P3M3);
    }

    /* Copy StoredCRC */
    memcpy(pRxBuffer, pDataParams->abStoredCRC, sizeof(pDataParams->abStoredCRC));  /* PRQA S 3200 */
